...
import { BUYER_COLORS, SYSTEM_COLORS } from '../../src/style/colors';
...
export default function BuyerProfileScreen() {
  ...
  const ambientShadow = {
    elevation: 2,
    shadowColor: BUYER_COLORS.onSurface, 
    shadowOpacity: 0.04,
    shadowRadius: 16,
    ...
    <View className="flex-1 bg-buyer-surface">
      ...
      >
        ...
        >
          ...
          <View className="w-20 h-20 rounded-xl bg-buyer-surface-container-highest border-2 border-white shadow-sm overflow-hidden z-10 flex items-center justify-center">
             <Text className="text-3xl font-black text-buyer-primary opacity-50">
          ...
          <View className="flex-1 z-10">
            <View className="flex-row items-center gap-2 mb-1">
              <Text className="text-2xl font-extrabold text-buyer-on-surface" numberOfLines={1}>
                {user?.fullName || 'Unknown Buyer'}
              </Text>
            </View>
            <View className="flex-row items-center bg-buyer-secondary-container self-start px-2 py-0.5 rounded mb-2">
              <MaterialIcons name="verified" size={12} color={BUYER_COLORS.onSecondaryContainer} />
              <Text className="text-[10px] font-bold text-buyer-on-secondary-container uppercase ml-1">Verified Buyer</Text>
            </View>
            

            <Text className="text-buyer-primary font-bold text-base mb-3" numberOfLines={1}>
            ...
            <View className="flex-row justify-between pr-4">
              <View>
                ...
                <Text className="text-xs font-semibold text-buyer-on-surface-variant">
                    {user?.role === 'ROLE_BUYER' ? 'Procurement' : 'Buyer'}
                </Text>
              </View>
              <View>
                <Text className="text-[9px] text-buyer-outline font-bold uppercase tracking-widest mb-0.5">Status</Text>
        ...
        <View className="bg-buyer-surface-container-high rounded-2xl p-6 mb-6 flex-row items-center justify-between">
          <View className="flex-row items-center gap-3 flex-1 pr-4">
            <View className="w-10 h-10 bg-white rounded-lg flex items-center justify-center">
              <MaterialIcons name="receipt-long" size={20} color={BUYER_COLORS.tertiary} />
            </View>
        ...
        <View className="bg-buyer-surface-container-lowest rounded-2xl border border-buyer-outline-variant/10 overflow-hidden mb-6" style={ambientShadow}>
          ...
          <Pressable className="flex-row items-center justify-between p-4 border-b border-buyer-outline-variant/10 active:bg-buyer-surface-container-low">
            <View className="flex-row items-center gap-4">
              <MaterialIcons name="import-contacts" size={20} color={BUYER_COLORS.outline} />
              <Text className="text-sm font-medium text-buyer-on-surface">Address Book</Text>
            </View>
            <MaterialIcons name="chevron-right" size={20} color={BUYER_COLORS.outlineVariant} />
          </Pressable>

          <Pressable className="flex-row items-center justify-between p-4 border-b border-buyer-outline-variant/10 active:bg-buyer-surface-container-low">
            <View className="flex-row items-center gap-4">
              <MaterialIcons name="credit-card" size={20} color={BUYER_COLORS.outline} />
          ...
          <Pressable className="flex-row items-center justify-between p-4 border-b border-buyer-outline-variant/10 active:bg-buyer-surface-container-low">
            <View className="flex-row items-center gap-4">
              <MaterialIcons name="task" size={20} color={BUYER_COLORS.outline} />
